<!-- ===========================================
     FerryReceiveLog.vue - Ferry Receive Log View
     =========================================== -->

<template>
  <div class="page-container">
    <PageHeader
      :title="t('audit.ferryReceive.title')"
    ...
    <SearchBar @search="handleSearch" @reset="handleReset">
      <el-form-item :label="t('audit.ferryReceive.user')">
        <el-input v-model="searchForm.userName" :placeholder="t('audit.ferryReceive.userPlaceholder')" clearable style="width: 130px" />
      </el-form-item>
      <el-form-item :label="t('audit.ferryReceive.ip')">
        <el-input v-model="searchForm.protocol" :placeholder="t('audit.ferryReceive.ipPlaceholder')" clearable style="width: 150px" />
      </el-form-item>
    ...
    >
      <template #REGION="{ row }">
        <el-tag :type="row.REGION === 'internal' ? 'success' : 'danger'" size="small" effect="dark">
          {{ row.REGION === 'internal' ? t('audit.ferryReceive.internal') : t('audit.ferryReceive.external') }}
        </el-tag>
      </template>
      <template #RESULT="{ row }">
...
const columns: TableColumn[] = [
  ...
  { prop: 'SENDUSER', label: '发送用户', width: 100 },
...
function generateMockData(): any[] {
  ...
  return Array.from({ length: 100 }, (_, i) => ({
    ...
    SENDUSER: users[Math.floor(Math.random() * users.length)],
    DESCRIBE: '文件接收操作日志记录'
  }))
}
